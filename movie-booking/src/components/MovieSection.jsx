...
const MovieSection = ({ title, movies }) => {
  ...
  const minSlidesRequired = isNowPlaying ? 2 : 4;
  const enableLoop = movies.length >= minSlidesRequired;
  ...
  return (
    <div className="mt-6 w-full">
      ...
      <Swiper
        ...
        slidesPerView={isNowPlaying ? Math.min(1.6, movies.length) : Math.min(3.2, movies.length)}
                ...
                >
                  <Link to={`/movies/${movie.id}`} state={{ movie }}>
                    <img
                      src={movie.image}
                      alt={movie.title}
                      className={`object-fill rounded-xl ${isNowPlaying ? "w-full h-[400px]" : "w-40 h-[150px]"}`}
                    />
                  </Link>
                </div>
                ...
                >
                  {isNowPlaying && movie.rating && (
                    <div className="bg-black/80 px-3 py-1 rounded-lg text-yellow-400 text-lg font-bold flex items-center justify-center">
                      ⭐ {movie.rating} <span className="text-sm text-gray-400 ml-2"></span>
                    </div>
                  )}
